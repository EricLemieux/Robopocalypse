...
private: 
	...
	// **** TAKE NOTES!
	//things in unions share memory. the below mean struct x,y,z occupies same memory space as vector3f u. Changing x means changing u.x because they're the same memory block
	union
	{
		struct {
			float x, y, z;
...
{
	// **** implement slerp
	//omega is angle between p0,p1
	//dot product of A and B = length(A) * length(B) * cos(omega)
	//omega = cosinv(dot(norm(a),norm(b)
	//[sin((1-t)*omega)/sin(omega)]  * p0
